...
public class FactorThread {    
	...
	public class FactorInterruptOriginal extends Thread {
		BigInteger n;
		BigInteger init;
		BigInteger stepSize;
		BigInteger result;
		
		public FactorInterruptOriginal (BigInteger n, int init, int stepSize) {
			this.n = n;
	...
	public static void main(String[] args) throws Exception {
		...
		FactorInterruptOriginal[] factors = new FactorInterruptOriginal[numberOfThreads];
		
		for (int i = 0; i < numberOfThreads; i++) {
			factors[i] = ft.new FactorInterruptOriginal(n, i+2, numberOfThreads);
			factors[i].start();
		}
